...
# URL for the Alfred camera feed (replace with your actual URL)
camera_url = "https://alfred.camera/webapp/viewer/device/1e2424fdb7f5f54f2d8d43e6b2110cf6/live?utm_source=webcamera&utm_medium=referral&utm_campaign=header_right"
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        thumb_fingertip = fingertip_coordinates[0]
        other_fingertips = fingertip_coordinates[1:3]

        distFingers = [calculate_distance(
            thumb_fingertip[:2], fingertip[:2]) for fingertip in other_fingertips]

        print(f"Distance: {distFingers}")

        if distFingers[1] > 700:  # Adjust this threshold as needed
            if not zoomed:
                pyautogui.hotkey('ctrl', '+')  # Zoom in
                zoomed = True
        elif distFingers[0] < 100:
            if not shrunk:
